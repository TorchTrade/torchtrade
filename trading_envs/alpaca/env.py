...
class CryptoTradingEnv(gym.Env):
    ...
    def _wait_for_next_timestamp(self) -> None:
        ...
        next_step_minutes = ((minutes_since_midnight // self.config.time_step_size) + 1) * self.config.time_step_size
    ...
    def step(self, action: np.ndarray) -> Tuple[Dict[str, np.ndarray], float, bool, Dict]:
        ...
        if self.config.done_on_bankruptcy:
            ...
            if new_portfolio_value < self.config.bankrupt_threshold * self.initial_portfolio_value:
                done = True
        ...
        info = {
            ...
            'portfolio_return': (new_portfolio_value - self.initial_portfolio_value) / self.initial_portfolio_value,
    ...
    def render(self, mode='human'):
        ...
        if mode == 'human':
            ...
            returns = (portfolio_value - self.initial_portfolio_value) / self.initial_portfolio_value
