...
class TestBybitFuturesTorchTradingEnv:
    ...
    def test_bankruptcy_termination(self, env, mock_trader, done_on_bankruptcy, expected_done):
        ...
        with patch.object(env, "_wait_for_next_timestamp"):
            env.reset()
            # AFTER reset: the baseline is re-captured per episode (#278), so an
            # episode that STARTS at 50 is a small episode, not a bankrupt one.
            # The collapse has to happen within the episode to mean anything.
            mock_trader.get_account_balance = MagicMock(return_value={
                "total_wallet_balance": 50.0,  # below 10% of the episode's own baseline
