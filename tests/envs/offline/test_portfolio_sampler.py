...
def test_listing_zeroes_features_and_blocks_trading_before_the_first_row():
    bars = make_portfolio_bars()
    late = (bars.inst_id == "A2") & (bars.timestamp < "2026-01-08")
    s = _sampler(bars[~late])
    a2 = s.inst_ids.index("A2")
    before = torch.from_numpy(np.flatnonzero(s.exec_times < "2026-01-08"))
    (key, _), = s.market_data_keys
...
@pytest.mark.parametrize("settlement,expected_step_offset", [
    ("fill", -1),       # exactly at T_n: belongs to the previous step
    ("inside", 0),      # strictly inside (T_n, T_{n+1}]
    ("next-fill", 0),   # exactly at T_{n+1}: still this step
])
def test_funding_window(settlement, expected_step_offset):
    s0 = _sampler(make_portfolio_bars())
    n = 5
    fill_n = s0.exec_times[n] + pd.Timedelta("4h")
    ts = {"fill": fill_n, "inside": fill_n + pd.Timedelta("1h"), "next-fill": fill_n + pd.Timedelta("4h")}[settlement]
    funding = pd.DataFrame({"timestamp": [ts], "inst_id": ["A0"], "funding_rate": [0.001]})
    s = _sampler(make_portfolio_bars(), funding=funding)
    charged = torch.nonzero(s.funding_exec[:, 0]).flatten().tolist()
    assert charged == [n + expected_step_offset]
