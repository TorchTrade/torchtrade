...
def test_liquidation_lands_on_the_breaching_bar_vectorized(sample_ohlcv_df):
    """Same pin as the scalar env's (#281), which this engine was built to mirror.

    Its docstring said "Matches scalar SequentialTradingEnv order: check liquidation at
    current bar (before trade)" -- so it inherited the bug deliberately, and the 1e-9
    equivalence harness kept them agreeing while both were wrong.
    """
    import pandas as pd
    ...
    env = VectorizedSequentialTradingEnv(df, VectorizedSequentialTradingEnvConfig(
        num_envs=1, action_levels=[-1.0, 0.0, 1.0], leverage=5, initial_cash=10000,
    ...
    for step in range(12):
        ...
        td = env.step(td)["next"]
        if float(td["account_state"][0][1]) == 0:
            liquidated_on = step
            break
    assert liquidated_on == 9, (
        f"liquidation observed on step {liquidated_on}, expected the breaching bar 9"
    )
    env.close()
