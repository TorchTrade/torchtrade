...
def _mocked_live_env(env_cls):
    """Build `env_cls` against mocked clients, or None if its config needs more than this.

    Deliberately generic rather than per-exchange: the point is that every live env gets
    the same done spec, so a helper that only fits one of them would not show that.
    """
    import numpy as np
    from unittest.mock import MagicMock, patch

    module = importlib.import_module(env_cls.__module__)
    config_cls = next(
    ...
    observer.get_observations = MagicMock(
        side_effect=lambda return_base_ohlc=False: (
            {"1Minute_10": np.zeros((10, 4), dtype=np.float32)}
            | ({"base_features": np.zeros((10, 4), dtype=np.float32)} if return_base_ohlc else {})
        )
    )
    trader = MagicMock()
    trader.get_status = MagicMock(return_value={"position_status": None})
    trader.get_account_balance = MagicMock(return_value={
        "total_margin_balance": 1000.0, "total_wallet_balance": 1000.0,
        "available_balance": 1000.0, "total_unrealized_profit": 0.0,
    })
    trader.get_mark_price = MagicMock(return_value=100.0)
    trader.client = MagicMock()
    trader.client.get_account = MagicMock(return_value=SimpleNamespace(cash="1000"))
    ...
    with patch.object(env_cls, "_wait_for_next_timestamp"):
        try:
...
@pytest.mark.parametrize("env_cls", NON_SLTP_ENVS, ids=lambda c: c.__name__)
def test_every_live_env_declares_truncated(env_cls):
    """The structural guard above says nobody re-declares it; this says it is there at all.

    A guard that only forbids a second copy passes just as happily when there is no first
    copy, which is the state all ten envs were in.
    """
    env = _mocked_live_env(env_cls)
    if env is None:
        pytest.skip(f"{env_cls.__name__} needs exchange-specific construction")

    assert "truncated" in env.full_done_spec.keys(), (
        f"{env_cls.__name__} does not declare truncated, but its _step writes one"
    )
