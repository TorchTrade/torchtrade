...
In this example we compare DQN and PPO on BTC/USD with 1Hour timeframe, `action_levels=[-1, 0, 1]`, `leverage=2`, and **no transaction fees** — purely to demonstrate general learning behavior. Note that without fees, policies typically overtrade since there is no cost to frequent position changes.
