...
1. **Validation**: The sampler checks that OHLCV + timestamp columns are present. Extra columns are accepted. For complete OHLC rows, it also raises `ValueError` on any bar where `high < max(open, close)` or `low > min(open, close)` — a bar whose extremes do not bracket its own open and close is rejected rather than carried, because the exit rules read the extreme to decide whether a level was touched, and the scalar stop-loss additionally reads the open to price a gapped fill — on such a bar those two disagree. Rows with a missing (`NaN`) OHLC value are incomplete rather than malformed by this relational check, so it does not reject them. Note that gap detection keys on `close` alone, so a bar missing only `open`, `high` or `low` is neither rejected nor warned about — see #353.
