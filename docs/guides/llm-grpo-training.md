...
adapter = LLMTrainer(
    ...
    num_generations=8,       # K completions per bar (the GRPO group); higher K = stronger signal
    logprob_chunk_size=1024, # chunk size for the memory-bounded log-prob/entropy computation
...
- Loss: stock torchrl `GRPOLoss` over a LoRA/QLoRA `ChunkedTransformersWrapper` (computes the
  log-prob/entropy from hidden states, plugged into the wrapper's dist hook so the loss is
